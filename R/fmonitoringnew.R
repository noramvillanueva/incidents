fmonitoringnew <- function(dia, mes, ano, tienda, maquina, dat, mititulo){
  ...
  y1 <- dat$ambiente[ dat$dia == dia & dat$mes == mes & 
                        dat$ano == ano & dat$tienda == tienda &
                        dat$maquina == maquina]
  color <- dat$inci[dat$dia == dia & dat$mes == mes & 
                      dat$ano == ano & dat$tienda == tienda &
...
#  yfinmax<-max(y1.1,y1.2,na.rm=TRUE)
  ...
  if(sum(!is.na(y1))!=0){
    plot(x, y1, cex = 0.5, pch = 16, xlab = "Hora", ylab = "Ambiente",
         ylim = c(min(y1, y1.2, na.rm = TRUE), 
          max(y1,y1.2, na.rm = TRUE)), col=color)
  ...
  if(sum(!is.na(y3))!=0){
    plot(x, y3, cex = 0.5, pch = 16, xlab = "Hora",main="Consumo",
         ylab = "Consumo",
         ylim = c(min(y3, na.rm = TRUE), max(y3, na.rm = TRUE)))
    points(x, y4, cex = 0.5, pch = 16,
           ylim = c(min(y3,y4, na.rm = TRUE), max(y3,y4, na.rm = TRUE)), 
           col="blue")
    legend("topleft",col=c(1,4), legend=c("General","Clima"),
           pch=16,  bty = "n")
  }
  #AGUA
  if(sum(!is.na(y7))!=0){
    ...
    points(x, y8, cex = 0.5, pch = 16,
           ylim = c(min(y7,y8, na.rm = TRUE), max(y7,y8, na.rm = TRUE)),
           col="blue")
    legend("topleft",col=c(1,4), legend=c("Impulsion","Retorno"), 
           pch=16,  bty = "n")
  }
  
  # AIRE
  if(sum(!is.na(y5))!=0&sum(!is.na(y6))!=0){
    plot(x, y5, cex = 0.5, pch = 16,  main='Aire', xlab = "Hora", 
         ylab = "Aire",
